...
public class ClubGrid {
   ...
   private ReentrantLock[] exitDoor;
	...
	ClubGrid(int x, int y, int [] exitBlocks,PeopleCounter c) throws InterruptedException {
      ...
      //---------------------------------------------
      //new instance to initialize 
      exitDoor = new ReentrantLock[exitBlocks.length];
      for(int i=0;i<exitBlocks.length;i++){
          exitDoor[i] = new ReentrantLock();
      }
      //---------------------------------------------
   }
...
//--------------------------------------------------------------------------------------
      ...
      public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
         synchronized (this) {
            while (ClubSimulation.currentInside >= ClubSimulation.max || entrance.occupied()) {
               wait(); // Wait if the club limit is reached
           }
      
        //only one patron to enter at a time
         entranceLock.lock(); 
...
//-----------------------------------------------------------------------------
      ...
      public  void leaveClub(GridBlock currentBlock,PeopleLocation myLocation){
        ...
        exitDoor[0].lock(); // Acquire the exit lock
        try{
   			currentBlock.release();
   			counter.personLeft(); //add to counter
   			myLocation.setInRoom(false);
            synchronized (this) {
               ClubSimulation.currentInside--;
               notifyAll(); // Notify waiting patrons that there's space available
            }
        }finally{
            exitDoor[0].unlock(); // Release the exit lock
        }
	   }
