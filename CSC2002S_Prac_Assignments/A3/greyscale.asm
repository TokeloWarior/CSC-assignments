	...
	# 																	      #
	# (i used WINDOWS 11 - DELL LATITUDE 3310 to run the code in QTSpim)	  #
	#																	 	  #
 ...
 .data
	...
	output_filename:.asciiz "C:/Users/TOKELO MAKOLOANE/OneDrive - University of Cape Town/2023 CURRICULUM/csc2002s/A3/final_code/output_images/output1.ppm"
	...
	#--------------------------------------------------------------------------------------------------------
		...
		li $t7, 0										# sum for the grey pixels
		li $t8,	0										# the reg to store sum of RGB 
		la $s1, storage
		...
		Blue_pixel_value:
			...
			store_Blue: 
				add $t8, $t8, $t2						# P = P + b
				div $t8, $t8, 3							# P = P/3
				mflo $t8								# P = the qoutient
				
				add $t6, $t6, $t2						# x = x + b
				
				add $t7, $t7, $t8						# y = y + P
				
				#converting_grey_pixel_to_string
				
				move $a0, $t8							# Load the integer into $a0
				...
				la $a0, string_driver
				jal concat								# concat the grey pixels for later use to store in file
